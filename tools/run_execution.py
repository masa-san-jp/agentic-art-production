...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("command", choices=("init", "replay", "record-output", "record-quality", "set-installation-plan", "record-installation-result", "record-evidence", "replay-evidence"))
    ...
    try:
        if args.command in {"record-evidence", "replay-evidence"}:
            ...
            if args.command == "replay-evidence":
                ...
                result = evidence.record_evidence(record, occurred_at=args.occurred_at, actor_kind=args.actor_kind, actor_id=args.actor_id, idempotency_key=args.idempotency_key)
        else:
