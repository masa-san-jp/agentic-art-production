...
from .lifecycle_guards import LifecycleGuardEvaluator
...
class Runtime:
    ...
    def __init__(self, project_root: Path, repository: Path):
        ...
        self._guard_evaluator: LifecycleGuardEvaluator | None = None

    def _lifecycle_guards(self) -> LifecycleGuardEvaluator:
        if self._guard_evaluator is None:
            self._guard_evaluator = LifecycleGuardEvaluator(self)
    ...
    def _check_transition_guard(
        ...
        *,
        ...
        return self._lifecycle_guards().evaluate(
            from_state,
            to_state,
            payload,
            actor,
            state or self._read_state(),
    ...
    def bootstrap(self, *, occurred_at: str, actor_kind: str, actor_id: str, idempotency_key: str = "runtime/bootstrap/1") -> dict[str, Any]:
        with self._writer_lock():
            ...
            payload["guard_evidence"] = self._check_transition_guard("HANDOFF_VALIDATED", "PLANNING", payload, actor, state=actual, occurred_at=occurred_at, event_sequence=1)
        ...
        with self._writer_lock():
            ...
            for event in events:
                if event["idempotency_key"] == idempotency_key:
                    ...
                    existing_base = deepcopy(existing_identity)
                    requested_base = deepcopy(requested_identity)
                    existing_base["payload"].pop("guard_evidence", None)
                    requested_base["payload"].pop("guard_evidence", None)
            ...
            actor = {"kind": actor_kind, "id": actor_id}
            if "guard_evidence" in transition_payload:
                raise DiagnosticError(_finding("RUNTIME_PAYLOAD_RESERVED", "guard_evidence is runtime-owned and cannot be supplied by callers", file=self.log_path, remediation="Let Runtime evaluate and bind canonical guard evidence to the transition event."))
            transition_payload["guard_evidence"] = self._check_transition_guard(from_state, to_state, transition_payload, actor, state=state, occurred_at=occurred_at, event_sequence=state["revision"] + 1)
